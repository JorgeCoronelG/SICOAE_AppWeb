$('#tabla-grado').DataTable({
    language: {
        ...
        "infoPostFix": "",
        "thousands": ",",
        "lengthMenu": "Mostrar _MENU_ Grado",
    ...
    'columns':[
        ...
        {'orderable': true,
            render: function(data, type, row){
                return '<button class="btn btn-outline-primary btn-circle" onclick="estadistica(\''+row.grado+'\');"><i class="fa fa-chart-bar"></i></button>';
            }
        }
    ],
    'columnDefs':[
...
function estadistica(grado){
    $.ajax({
        url: '/registro/grade/all/'+grado,
        type: 'get',
        dataType: 'json',
        beforeSend: function(){
            $('.btn').addClass('d-none');
        ...
        error: function(jqXHR){
            ...
            if(errors.hasOwnProperty('grado')) toastr.error(errors['grado'][0], 'Error');
        },
        complete: function(){
